define('app/map', ['app/area'], function (Area) {
    
    var Map = new Class({

        initialize: function (loadMultiTilesheets, game) {
            this.game = game;
        	this.data = [];
        	this.isLoaded = false;
        ...
        _getDoors: function(map) {
            ...
            Array.each(map.doors, function(door) {
                var o;
        ...
        _generateCollisionGrid: function() {
            ...
            */
            ...
            Array.each(this.collisions, function(tileIndex) {
                var pos = self.tileIndexToGridPosition(tileIndex+1);
                self.grid[pos.y][pos.x] = 1;
            });

            Array.each(this.blocking, function(tileIndex) {
                var pos = self.tileIndexToGridPosition(tileIndex+1);
                if(self.grid[pos.y] !== undefined) {
                    self.grid[pos.y][pos.x] = 1;
                }
        ...
        _getCheckpoints: function(map) {
            ...
            */
            Array.each(map.checkpoints, function(cp) {
                var area = new Area(cp.x, cp.y, cp.w, cp.h);
                area.id = cp.id;
                checkpoints.push(area);
            });
